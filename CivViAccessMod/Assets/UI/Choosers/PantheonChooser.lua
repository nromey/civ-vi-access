...
--
-- ScreenReaderAccess shadow: minimal additions over the base PantheonChooser —
--   1. include("ChoosePopupAccess") at top.
--   2. Open() tail builds the accessible option list (the same beliefs the
--      visual list shows) and hands it to ChoosePopupAccess.Open with a commit
--      callback that founds the pantheon directly.
--   3. Close calls ChoosePopupAccess.NotifyClose.
--   4. OnInputHandler routes through ChoosePopupAccess.HandleKey first.
-- All other code is verbatim from the base game.
include("InstanceManager");
include("ChoosePopupAccess");   -- begin/end ScreenReaderAccess mod change
...
-- Build the accessible option list from the SAME beliefs the visual list shows
-- (GameInfo.Beliefs filtered by CanSelectBelief), each carrying its belief hash
-- as opaque data, and hand it to ChoosePopupAccess. Commit founds the pantheon
-- directly (the same RequestPlayerOperation ConfirmPantheon makes) so we don't
-- depend on the mouse-driven instance selection.
function CommitPantheonBelief( beliefHash:number )
	local tParameters:table = {};
	tParameters[PlayerOperations.PARAM_BELIEF_TYPE] = beliefHash;
...
function RealizeAccessibleChooser()
	local options:table = {};
	for row in GameInfo.Beliefs() do
		if CanSelectBelief(row) then
	...
	ChoosePopupAccess.Open({
		title    = Locale.Lookup("LOC_UI_RELIGION_CHOOSING_PANTHEON"),
		options  = options,
		typeNoun = "belief",
		onCommit = function(opt) CommitPantheonBelief(opt.data); end,
