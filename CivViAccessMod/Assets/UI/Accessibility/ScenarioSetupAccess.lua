-- ScenarioSetup accessibility companion.
--
-- Mirror of AdvancedSetupAccess for Civ VI's ScenarioSetup screen
-- (Single Player → Scenarios → pick scenario). The base screen uses
-- the SAME parameter framework as AdvancedSetup — g_GameParameters,
-- the same per-slot SetupParameters via GetPlayerParameters, the
-- same Array / Domain / Values shapes — so the item-building logic
-- is identical. The only screen-specific deltas are the BaseMenu
-- `name` / `displayName` and the LOC key used for the screen title.
--
-- Why a near-duplicate file instead of factoring out: until a third
-- consumer of this exact shape appears (multiplayer staging room is
-- a likely candidate), duplicating is lower risk than building a
-- shared module. The diff vs AdvancedSetupAccess is intentionally
-- minimal — keep them aligned when fixing bugs in either.
--
-- Wires keyboard navigation + spoken focus tracking onto Civ VI's
-- ScenarioSetup screen via BaseMenu. The base screen is parameter-driven
...
-- function that re-reads the parameter list on every show — picking a
-- different scenario reshapes the set between shows.
--
-- Top-level shape:
...
-- Civ VI's ScenarioSetup uses named globals for its handlers (OnShow, OnHide,
...
-- can chain them — this file is included at the bottom of the ScenarioSetup
...
local function parameterItem(parameter, gameParameters)
    ...
    -- cue that Enter opens a modal. ScenarioSetup doesn't set
...
local function topLevelItems()
    ...
    if g_GameParameters == nil or g_GameParameters.Parameters == nil then
        ...
        -- ScenarioSetup-specific: force the Scenario pulldown to the
        -- very top regardless of its SortIndex. It's the most important
        -- choice on this screen — every other parameter's allowed values
        -- depend on which scenario is selected — so it deserves the
        -- "Home key lands here" position.
        local function sortKey(p)
            return (p.SortIndex or 0) * 1000000 + 0
        end
        table.sort(globalParams, function(a, b)
            -- Pin Scenario first.
            if a.ParameterId == "Scenario" then return true end
            if b.ParameterId == "Scenario" then return false end
...
local _menuHandler = BaseMenu.install(ContextPtr, {
    name = "ScenarioSetup",
    displayName = Locale.Lookup("LOC_CIVVIACCESS_SCREEN_SCENARIO_SETUP"),
    items = topLevelItems,
    -- Scenarios is a "browse and pick" screen — the Scenario pulldown
    -- is the dominant interaction, and the per-value description is
    -- the only way to tell scenarios apart. Always chatty here so
    -- arrowing onto Scenario reads the description immediately;
    -- AdvancedSetup deliberately stays terse at L1 because its
    -- parameter tooltips are mostly generic ("Choose the ruleset…").
    alwaysVerbose = true,
    preamble = function()
...
function UI_PostRefreshParameters()
    if _priorPostRefresh ~= nil then
        ...
        if not ok then
            print("[ScenarioSetupAccess] priorPostRefresh failed: " .. tostring(err))
        end
    end
