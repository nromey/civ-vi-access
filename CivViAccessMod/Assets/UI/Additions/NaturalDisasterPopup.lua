...
--      header + name + description + impact counts (units lost, pop
--      lost, tiles damaged, fertility delta, mitigated flag).
...
function ShowPopup( kData:table )
	...
	local sHeader:string;
	if kData.EffectOperatorType == NUCLEAR_OPERATOR_TYPE then
		sHeader = Locale.ToUpper(Locale.Lookup("LOC_UI_FEATURE_NUCLEAR_DISASTER_DISCOVERY"));
		Controls.HeaderTitle:SetText(Locale.ToUpper("LOC_UI_FEATURE_NUCLEAR_DISASTER_DISCOVERY"));
	else
		sHeader = Locale.ToUpper(Locale.Lookup("LOC_UI_FEATURE_NATURAL_DISASTER_DISCOVERY"));
	...
	-- begin ScreenReaderAccess mod change
	-- Assemble header + name + description + impact counts into one
	-- announce. Counts are appended only when non-zero (mirrors the
	-- UI's container hide/show logic above).
	local parts = {};
	if sHeader ~= nil and sHeader ~= "" then
		table.insert(parts, sHeader);
	end
	...
	if kData.IsMitigated then
		table.insert(parts, Locale.Lookup("LOC_UI_DISASTER_MITIGATED"));
	end
	if kData.UnitsLost ~= nil and kData.UnitsLost > 0 then
		table.insert(parts, tostring(kData.UnitsLost) .. " units lost");
	end
	if kData.PopLost ~= nil and kData.PopLost > 0 then
		table.insert(parts, tostring(kData.PopLost) .. " population lost");
	end
	if kData.TilesDamaged ~= nil and kData.TilesDamaged > 0 then
		table.insert(parts, tostring(kData.TilesDamaged) .. " tiles damaged");
	end
	if kData.FertilityAdded ~= nil and kData.FertilityAdded < 0 then
		table.insert(parts, tostring(math.abs(kData.FertilityAdded)) .. " fertility lost");
	end
	if kData.FertilityAdded ~= nil and kData.FertilityAdded > 0 then
		table.insert(parts, tostring(kData.FertilityAdded) .. " fertility added");
	end
