...
-- ===========================================================================
-- MainMenuAccess: inlined from former Assets/UI/Accessibility/MainMenuAccess.lua.
--
-- Originally lived in a separate file so the MainMenu fork stayed small,
-- but Civ VI's include() returns from a process-wide cache on the second
-- MainMenu context load (e.g. after exit-to-main-menu) without re-executing
-- the file. Result: the MainMenuAccess global was set on the first load
-- but nil on every subsequent load, and main-menu kb access silently died
-- whenever the user came back to the menu mid-session. Confirmed via
-- Lua.log 2026-05-24 diagnostic. Inlining moves the table construction
-- into MainMenu.lua's own top-level scope, which DOES re-run per context.
--
-- See memory: [[project_05_session_handoff_2026_05_24]] item #24.
-- ===========================================================================

print("[CivViAccess][INFO ] MainMenu.lua: building MainMenuAccess inline");
...
print("[CivViAccess][INFO ] MainMenu.lua: MainMenuAccess inline build complete");
...
function BuildMenu(menuOptions:table)
	...
	-- Hand the kb-accessibility companion a flat {control, activate} list
	-- mirroring the same closure semantics as the Mouse.eLClick callback
	-- above, so Enter on a focused item runs the same code path the mouse
	-- click does (including submenu propagation).
	do
		local accessOptions = {};
		for i, menuOption in ipairs(menuOptions) do
			accessOptions[i] = {
				control = m_currentOptions[i].control,
				activate = function()
					if (menuOption.submenu ~= nil) then
						menuOption.callback(i, menuOption.submenu);
					else
...
function BuildSubMenu(menuOptions:table)
	...
	for i, kMenuOption in ipairs(menuOptions) do
		...
		if kMenuOption.space then
			...
			accessSubOptions[#accessSubOptions + 1] = {
				control  = uiOption,
				activate = kMenuOption.callback,
			};
		end
	end
