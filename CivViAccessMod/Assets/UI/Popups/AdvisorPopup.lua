...
--	announce + keyboard input dispatch. See
--	docs/flow-trace/06-first-turn-advisor-popup.md and
--	Accessibility/AdvisorPopupAccess.lua.)
--
--	Only three edits from the Firaxis original:
--	  1. include("AdvisorPopupAccess") at the top
--	  2. ShowAdvisorPopup tail calls AdvisorPopupAccess.NotifyShow
--	  3. Close + OnHideAdvisorDialog call AdvisorPopupAccess.NotifyClose
--	  4. OnInputHandler asks AdvisorPopupAccess.HandleKey first
-- ===========================================================================
include( "InstanceManager" );
include( "Civ6Common" ); -- IsTutorialRunning()
include( "AdvisorPopupAccess" );
...
function ShowAdvisorPopup( advisorData:AdvisorItem )
	...
	-- Civ VI Access hand-off: announce + arm input dispatch. Runs AFTER
	-- engine setup so the popup state is real when we read it.
	if AdvisorPopupAccess ~= nil and AdvisorPopupAccess.NotifyShow ~= nil then
		AdvisorPopupAccess.NotifyShow(advisorData);
	end
...
function Close()
	...
	if AdvisorPopupAccess ~= nil and AdvisorPopupAccess.NotifyClose ~= nil then
		AdvisorPopupAccess.NotifyClose();
	end
end
...
function OnInputHandler( pInputStruct:table )
	-- Civ VI Access first: arrow nav, Enter on focused button, Esc
	-- override, Ctrl+I, Ctrl+T. Returns true if it consumed the input.
	if AdvisorPopupAccess ~= nil and AdvisorPopupAccess.HandleKey ~= nil then
		if AdvisorPopupAccess.HandleKey(pInputStruct) then
			return true;
		end
	end
...
-- Wrapper for ShowAdvisorPopup. Calls NotifyShow EARLY, before the
-- original rendering body runs, so the access layer is armed (m_visible
-- = true, m_currentData populated, m_buttonCount counted) regardless
-- of whether the rendering body completes or errors. Round 2 found
-- the original rendering body running to ShowAdvisorPopup INVOKED but
-- never reaching the NotifyShow call at the very end — meaning if our
-- NotifyShow hand-off lived at the end of the original function it
-- would silently never fire. Pre-arming the access layer here means
-- HandleKey always knows the popup is open and can intercept arrows /
-- Enter / Esc / Ctrl+T/I correctly even if the original errors mid-
-- rendering. The bottom-of-original NotifyShow call is now harmless
-- (becomes a no-op since state is already set) so we leave it as
-- belt-and-suspenders.
local _origShowAdvisorPopup = ShowAdvisorPopup;
function ShowAdvisorPopup( advisorData )
	print("[CivViAccess][INFO ] ShowAdvisorPopup INVOKED data=" ..
	      tostring(advisorData ~= nil) ..
	      " portrait=" .. tostring(advisorData and advisorData.ShowPortrait or "?") ..
	      " msg=" .. tostring(advisorData and advisorData.Message or "?") ..
	      " b1=" .. tostring(advisorData and advisorData.Button1Text or "?") ..
	      " b2=" .. tostring(advisorData and advisorData.Button2Text or "?"));
	-- Round-4 diagnostics: round-3 log showed this wrapper firing but
	-- NotifyShow's Log.info never appearing. Surface every step so we
	-- can see exactly where the chain breaks.
	print("[CivViAccess][INFO ] AdvisorPopupAccess type=" .. type(AdvisorPopupAccess));
	if type(AdvisorPopupAccess) == "table" then
		print("[CivViAccess][INFO ] AdvisorPopupAccess.NotifyShow type=" .. type(AdvisorPopupAccess.NotifyShow));
	end
	if AdvisorPopupAccess ~= nil and AdvisorPopupAccess.NotifyShow ~= nil then
		print("[CivViAccess][INFO ] About to call NotifyShow");
		local ok, err = pcall(AdvisorPopupAccess.NotifyShow, advisorData);
		if not ok then
			print("[CivViAccess][ERROR] NotifyShow threw: " .. tostring(err));
		else
			print("[CivViAccess][INFO ] NotifyShow returned successfully");
		end
	else
		print("[CivViAccess][WARN ] SKIPPED NotifyShow: AdvisorPopupAccess or NotifyShow is nil");
	end
	return _origShowAdvisorPopup(advisorData);
end
